...
1. 真实微调引擎剩余里程碑（M1、M2 已 PASS）：
   ...
   - **M5** 收尾：真实 GPU 利用率采样替换 `train_metric.gpu=0`、断点续训、OOM 处理、CUDA 镜像（torch 2.12.1+cu132 + LLaMA-Factory 0.9.5，宿主仅驱动）、超参「应用到任务」前端入口。
...
**优先级建议**：~~① 上线/回滚/灰度 + 评估闭环~~（✅）→ ~~② 数据集脱敏/标注/版本/权限 + 配额/调优~~（✅）→ ~~③ 接口层 RBAC + 用户管理~~（✅）→ ~~收尾项（文件上传/导出/部署/归档/调度持久化/Alembic/Docker）~~（✅，2026-06-22）→ ~~收尾小项（日志下载/超参存模板/新建版本/上传校验）~~（✅，2026-06-23）→ ④ 真实微调引擎 ~~M1 单卡最小闭环~~（✅，2026-06-23）→ M2-M5（converter/控制面/产物/CUDA镜像）。
