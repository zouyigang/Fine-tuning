...
- **真实微调引擎 M1（2026-06-23）**：方案见 `docs/真实微调引擎方案.md`。本机环境已就绪（RTX 5080/Blackwell sm_120，torch 2.11.0+cu130，llamafactory 0.9.4.dev0，transformers 4.52.4），**真跑 Qwen3-0.6B LoRA 验收 PASS**（36 个真实指标点，loss 1.54→0.006，自动建 model_version）。
  - **开关**：`settings.ENGINE_MODE`（默认 `sim`=旧模拟调度器；`real`=真实引擎）。`real` 时 `main.py` 启 `services/engine.py`，`createTask` 默认 `pending` 入队。**默认 sim，现有部署零影响**。
  - **新配置**：`ENGINE_MODE`/`MODELS_DIR`(离线 HF 权重根)/`RUNS_DIR`/`LLAMAFACTORY_BIN`/`MAX_CONCURRENT_TRAINS`。
  - **DB 改动（Alembic `0002_real_engine`，inspector 幂等，兼容全新/既有库）**：`train_task` 加 `method/hyperparams(JSON)/base_model_path/output_dir/pid/error_msg/model_version_id/finished_at`；`model_version` 加 `task_id`；新表 `task_artifact`。
  - **`services/engine_config.py`**：`resolve_model_path`(剥「（开源）」后缀找 `MODELS_DIR/<名>/config.json`)、`register_dataset`(M1 标准格式 alpaca/sharegpt/{prompt,response}→注册进 `RUNS_DIR/_lf_data/dataset_info.json`，拒非标)、`build_train_yaml`(超参→LF YAML，batch 拆 per_device+grad_accum，qlora→quantization_bit:4)、`ensure_demo_dataset`(自带极小 demo 回退)。
  - **`services/engine.py`** `TrainingManager`：调度线程按 `MAX_CONCURRENT_TRAINS` 取 pending→`subprocess` 跑 `llamafactory-cli train`（`CUDA_VISIBLE_DEVICES`）→监控线程读 `output_dir/trainer_log.jsonl` 回流 `train_metric`、stdout 入 `train_log`→退出码 0 则 success+建 `model_version`(evaluating)+写 `task_artifact`，否则 failed+`error_msg`；`stop_task` kill 子进程（task 状态改 stopped/paused 时触发）。
  - **超参透传**：`create.vue` 不再丢弃，`TaskCreate` 带 `method`+`hyperparams`。前端 `BASE_MODELS` 加 Qwen3-0.6B。requirements 加 `PyYAML`/`nvidia-ml-py`。SQLite 加 `timeout=30` 防多线程 locked。

**待办（更新于 2026-06-23）**

> 说明：以下「占位」= 前端有按钮但只弹 toast / setTimeout 假成功，未落库；「缺写接口」= 后端只有 GET 没有对应写接口。

1. 真实微调引擎里程碑（M1-M5 全部完成）：
   - ~~**M2** 数据 converter~~（✅ 2026-06-23）：`services/dataset_convert.py` 把业务原始 schema → alpaca 指令样本。按数据集 `type`(OCR校对/实体关系/事件/风险) 选专用转换器（字段别名宽松识别、坏行跳过），不匹配走通用兜底（问答键/两字段对）。`register_dataset` 非标准格式时调它转换后落 `*_converted.jsonl` 再注册；标准格式仍直通。单测 10/10。
   - ~~**M3** 控制面~~（✅ 2026-06-23）：真实停止/暂停/继续/重试接前端（`updateTaskStatus` 后端按 real 翻译：running=重置入队 pending 由调度器重启；paused/failed/stopped=kill 子进程并记「主动目标态」，收尾 `_mark` 按目标态落库、不误判 failed/不建版本）；`engine.gpu_count()`（pynvml→torch→1 回退）+ `concurrency()=min(上限,GPU)` 动态并发 + `_pick_device` 多卡分配 `CUDA_VISIBLE_DEVICES`；`GET /task/gpus` 实时 GPU 状态（pynvml）。`crud.requeue_task` 重置 progress/pid/error。逻辑验证 5/5。
   - ~~**M4** 产物~~（✅ 2026-06-23，真跑 PASS）：LoRA/QLoRA 训练成功后引擎自动 `llamafactory-cli export` 合并 adapter 进基座，输出完整权重到 `RUNS_DIR/{id}/merged`，写 `task_artifact`(adapter + merged 两条)、`model_version.size` 取合并权重大小。`engine_config.build_export_yaml` 生成合并配置；`engine._merge_export`(best-effort，失败仅 WARN 不影响版本)。导出/下载串联：`crud.real_artifacts`/`_merged_artifact`，真实模型(`model_version.task_id` 有关联)导出 manifest 标 `real=true` 并引用真实 `weightsDir`+文件清单+大小（多 GB 权重不打包进单文件，manifest 作描述符指向真权重），种子模型仍占位。`GET /model/{id}/artifacts` + 前端 `getModelArtifacts`。注：`success` 早于 merge 完成落库，merged 产物约数十秒后出现（live server 守护线程完成提交）。
   - ~~**M5** 收尾~~（✅ 2026-06-23）：① 真实 GPU 利用率采样（`engine._sample_gpu(device)` pynvml→`train_metric.gpu` 写真实利用率，无 pynvml 降级 0）；② 断点续训（`engine_config.has_checkpoint` 检测 `checkpoint-*`，`build_train_yaml(resume=True)` → `resume_from_checkpoint:true`+`overwrite_output_dir:false`，`_launch` 重试时自动续训）；③ OOM 处理（`_pump_stdout` 扫 "out of memory" 标记 `_oom`，`_finish` 给明确提示「改用 QLoRA/减 batch/缩短序列」）；④ CUDA 镜像（`server/Dockerfile.cuda` python3.12-slim+torch2.12.1+cu132+llamafactory0.9.5 + `docker-compose.gpu.yml` override：`gpus:all`+`ENGINE_MODE=real`+挂载 `${MODELS_DIR}`，宿主仅驱动；用 `docker compose -f docker-compose.yml -f docker-compose.gpu.yml up -d --build`）；⑤ 超参「应用到任务」（`hyperparams.vue`「应用到任务」选 pending/非运行任务 → `PUT /task/{id}/hyperparams`→`crud.apply_hyperparams`，运行中/已完成拒改，oplog 加规则）。逻辑验证 7/7。
...
**优先级建议**：~~① 上线/回滚/灰度 + 评估闭环~~（✅）→ ~~② 数据集脱敏/标注/版本/权限 + 配额/调优~~（✅）→ ~~③ 接口层 RBAC + 用户管理~~（✅）→ ~~收尾项（文件上传/导出/部署/归档/调度持久化/Alembic/Docker）~~（✅，2026-06-22）→ ~~收尾小项（日志下载/超参存模板/新建版本/上传校验）~~（✅，2026-06-23）→ ~~④ 真实微调引擎 M1-M5（单卡闭环/converter/控制面/产物/收尾+CUDA镜像）~~（✅，2026-06-23）。**真实微调引擎全部完成**，剩平台级 Token 刷新 / RBAC 收紧。
